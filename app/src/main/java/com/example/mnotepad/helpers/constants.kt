package com.example.mnotepad.helpers

const val BASE_URL = "https://dummyjson.com"
const val USER_DETAIL_ID = "user_detail_id"
const val NOTE_DETAIL_OBJECT = "note_detail_object"
const val CATEGORY_ID = "category_id"
const val IS_EDITED_ACTION = "is_edited_action"
const val SORT_EDIT_DATE_FROM_NEWEST = "edit date: from newest"
const val SORT_EDIT_DATE_FROM_OLDEST = "edit date: from oldest"
const val SORT_TITLE_A_Z = "title: A to Z"
const val SORT_TITLE_Z_A = "title: Z to A"
const val SORT_CREATE_DATE_FROM_NEWEST = "creation date: from newest"
const val SORT_CREATE_DATE_FROM_OLDEST = "creation date: from oldest"
const val SORT_COLOR = "color: in order as shown on color palette"

const val PREFS_NAME = "app_prefs"
const val PLAIN_TYPE = "text/plain"
const val DELAY_TYPING: Long = 2000
const val CATEGORY_MENU_ID = 5
const val DEFAULT_UNLOCK_TIME = 10
const val FIVE_UNLOCK_TIME = 5
const val TEN_UNLOCK_TIME = 10
const val FIFTEEN_UNLOCK_TIME = 15
const val THIRTY_UNLOCK_TIME = 30

const val THEME_BROWN_ID = 0
const val THEME_BLACK_ID = 1
const val THEME_YELLOW_ID = 2
const val THEME_GREY_ID = 3
const val THEME_SYSTEM_ID = 4

const val INCORRECT_PASSWORD = "Incorrect password"
const val INCORRECT_OLD_PASSWORD = "Incorrect old password"
const val INVALID_NEW_PASSWORD = "Invalid new password"
const val RECOVERY_EMAIL_REQUIRED = "Recovery Email is required"

const val UNCATEGORIZE = "Uncategorize"
const val PALE_VALUE = 0.5f
